@php
    ...
    $tableRowData = $collection->map(function ($university) {
        return [
            'id' => $university->id,
...
<div x-data="{
    ...
    universityBaseUrl: {{ \Illuminate\Support\Js::from(url('/' . $role . '/universities')) }},
    ...
    confirmDelete() {
        ...
        <form x-ref="deleteForm" :action="rowToDelete ? (universityBaseUrl + '/' + rowToDelete.id) : '#'" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
        ...
        <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[99999]">
            ...
            <div class="absolute inset-0 flex items-center justify-center p-4">
                <div class="w-full max-w-md rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-xl">
                    <div class="p-5">
                        <div class="text-base font-semibold text-gray-800 dark:text-white/90">Delete University?</div>
                        <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            This will permanently delete university:
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                                ...
                                <td class="px-5 py-4">
                                    <template x-if="row.logo">
                                        <img :src="row.logo" class="w-10 h-10 rounded border border-gray-200 object-contain" alt="logo" loading="lazy">
                                    </template>
                                    <template x-if="!row.logo">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <a :href="universityBaseUrl + '/' + row.id + '/edit'" class="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-all">
